 ...
 * Writing the format once, against the same {@link MCPluginManagerAction} the Bukkit
 ...
 */
public final class MCPluginManagerPayloadCodec {

    /**
     * Not instantiable.
     */
    private MCPluginManagerPayloadCodec() {
    }
     ...
     */
    public static byte[] encodeRequest(MCPluginManagerRequest request) {
     ...
     *         version of the mod.
     */
    public static MCPluginManagerRequest decodeRequest(byte[] bytes) {
        try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
            UUID playerId = new UUID(in.readLong(), in.readLong());
            String action = in.readUTF();
            String payload = in.readUTF();
            return new MCPluginManagerRequest(playerId, MCPluginManagerAction.valueOf(action), payload);
     ...
     * @return The bytes to send.
     */
    public static byte[] encodeResponse(MCPluginManagerResponse response) {
        ByteArrayOutputStream buffer = new ByteArrayOutputStream();
        try (DataOutputStream out = new DataOutputStream(buffer)) {
     ...
     */
    public static MCPluginManagerResponse decodeResponse(byte[] bytes) {
        try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
            return new MCPluginManagerResponse(in.readBoolean(), in.readUTF());
        } catch (IOException e) {
