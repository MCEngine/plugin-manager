package io.github.mcengine.pluginmanager.common;

import io.github.mcengine.pluginmanager.api.AbstractMCPluginManagerService;
...
/**
 * the in-memory implementation this plugin ships with.
 *
 ...
 */
public final class DefaultMCPluginManagerService extends AbstractMCPluginManagerService {

    /**
     * The greeting shown to a player who has not set one.
     */
    private static final String DEFAULT_GREETING = "Hello from MCPluginManager";
     ...
     */
    @Override
    protected CompletableFuture<MCPluginManagerResponse> ping(MCPluginManagerRequest request) {
        return CompletableFuture.completedFuture(running
            ? MCPluginManagerResponse.accepted("pong")
            : MCPluginManagerResponse.rejected("The service is not running"));
    }
     ...
     */
    @Override
    protected CompletableFuture<MCPluginManagerResponse> greet(MCPluginManagerRequest request) {
        if (!running) {
        ...
        return CompletableFuture.completedFuture(
            MCPluginManagerResponse.accepted(greetingFor(request.playerId()).orElse(DEFAULT_GREETING)));
    }
