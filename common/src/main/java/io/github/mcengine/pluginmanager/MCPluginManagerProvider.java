...
import io.github.mcengine.pluginmanager.api.MCPluginManagerService;
import io.github.mcengine.pluginmanager.common.DefaultMCPluginManagerService;
 ...
 * <h2>Typical use</h2>
 * <pre>{@code
 * MCPluginManagerProvider provider = MCPluginManagerProvider.instance;
 * provider.handle(MCPluginManagerRequest.of(playerId, MCPluginManagerAction.GREET))
 *         .thenAccept(response -> player.sendMessage(response.message()));
 * }</pre>
 */
public final class MCPluginManagerProvider implements MCPluginManagerService {
     ...
     * region threads.</p>
     */
    public static volatile MCPluginManagerProvider instance;
     ...
     */
    private final MCPluginManagerService service;
     ...
     */
    public MCPluginManagerProvider(MCPluginManagerService service) {
     ...
     */
    public static MCPluginManagerProvider create() {
        return new MCPluginManagerProvider(new DefaultMCPluginManagerService());
    }
     ...
     */
    @Override
    public CompletableFuture<MCPluginManagerResponse> handle(MCPluginManagerRequest request) {
